...
class Status extends Page
{
    ...
    protected function getViewData()
    {
        if(isset($_GET['order'])){
			...
			$stat = $this->_database->prepare('SELECT angebot.name, angebot_bestellung.status FROM angebot_bestellung
			INNER JOIN angebot ON angebot.id = angebot_bestellung.angebot_id
			WHERE angebot_bestellung.bestellung_id = ?');
			$stat->bind_param('i', $order);
			if ($stat->execute()) {
				$stat->bind_result($name, $status);
    ...
    protected function generateView()
    {
        ...
        $this->statusTabelle->generateView('status', null, $columns, $this->_order);
